<%@page import="kr.or.iei.notice.model.vo.Notice"%>
<%@page import="java.util.ArrayList"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%
    	ArrayList<Notice> list=(ArrayList<Notice>)request.getAttribute("list");
    	String pageNavi=(String)request.getAttribute("pageNavi");
    %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
	<style>
		.notice-tbl a:hover{
			text-decoration:underline;
		}
		.notice-tbl tr{
			border-bottom:1px solid #ccc;
		}
		.notice-tbl th{
			background-color:lightgray;
		}
		.notice-tbl tr>th:first-child{
			width:10%;
		}
		.notice-tbl tr>th:nth-child(2){
			width:45%;
		}
		.notice-tbl tr>td:nth-child(2){
			text-align:left;
		}
		.notice-tbl tr>th:nth-child(3){
			width:15%;
		}
		.notice-tbl tr>th:nth-child(4){
			width:20%;
		}
		.notice-tbl tr>th:last-child{
			width:10%;
		}
	</style>
</head>
<body>
	<%@include file="/WEB-INF/views/common/header.jsp" %>
	<!-- <form name="msgWriteFrm" action="/writeMsgFrm.do">
			<input type="hidden" name="receiver">
		</form> -->
		
	<div class="page-content">
		<div class="page-title">공지사항</div>
		<%if(m!=null&&m.getMemberLevel()==0){ %>
		<a class="btn bc4 writeBtn" href="/noticeWriteFrm.do">글쓰기</a>
		<%} %>
		<table class="tbl tbl-hover notice-tbl">
			<tr>
				<th>번호</th><th>제목</th><th>작성자</th><th>작성일</th><th>조회수</th>
			</tr>
			<%for(Notice n : list){ %>
			<tr class="tr-1">
				<td><%=n.getNoticeNo() %></td>
				<td><a href="/noticeView.do?noticeNo=<%=n.getNoticeNo() %>"><%=n.getNoticeTitle() %></a></td>
				<td>
					<%if(m==null||m.getMemberId().equals(n.getNoticeWriter())) {%>
					<%=n.getNoticeWriter() %>
					<%}else{ %>
					<a href="/writeMsgFrm.do?memberId=<%=n.getNoticeWriter() %>">
					<!-- <a id="msg"> -->
					<%=n.getNoticeWriter() %>
					</a>
					<%} %>
				</td>
				<td><%=n.getReqDate() %></td>
				<td><%=n.getReadCount() %></td>
			</tr>
			<%} %>
		</table>
		<div id="pageNavi"><%=pageNavi %></div>
	</div>
	<!-- <script>
	$("td>#msg").on("click",function(){
		const memberId=$(this).text();
		$("[name=receiver]").val(memberId);
		const popup=window.open("","receiver","left:700px, top=300px,width:300px, height=200px,menubar=no, status=no,scrollbars=yes")
		$("[name=msgWriteFrm]").attr("target","receiver");
		console.log($("[name=msgWriteFrm]").val());
		$("[name=msgWriteFrm]").submit();
	})
	</script> -->
	<%@include file="/WEB-INF/views/common/footer.jsp"%>
</body>
</html>
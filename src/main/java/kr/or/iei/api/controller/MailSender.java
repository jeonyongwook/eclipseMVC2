...
public class MailSender {
	...
	public boolean sendMail(String mailTitle, String mailContent, String receiver) {
		...
		Session session = Session.getDefaultInstance(prop, new Authenticator() {
			protected PasswordAuthentication getPasswordAuthentication() {
				PasswordAuthentication pa = new PasswordAuthentication("wookie3803@gmail.com", "jyy185632");
				return pa;
			}
		});
	...
	public String sendMail(String email) {
		...
		try {
			...
			// 이메일 본문설정
			msg.setContent("<h1>안녕하세요 kh정보교육원입니다</h1>"
			+"<h3>인증번호는 [<span style='color:red;'>"+sb.toString()+"</span>]입니다.</h3>", "text/html;charset=UTF-8");
			// 이메일 전송
			Transport.send(msg);
			// 전송이완료되면 전송결과 값 true로 변경
			result = true;
